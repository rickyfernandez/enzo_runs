...
for file in file_names:
    ...
    plot = yt.ProfilePlot(sphere, "radius", ["metallicity"],
            weight_field="cell_mass")
    profile = plot.profiles[0]
    plt.loglog(profile.x/pc_to_cm, profile["metallicity"])
    labels.append(r"%0.2f Myr" % ds.current_time.value)

plt.xlabel(r"Radius $(\mathrm{pc})$")
plt.ylabel(r"Metallicity $(Z_{\odot})$")
plt.xlim(1,75)
plt.legend(labels, loc="lower left", frameon=False)
plt.savefig("metallicity_series")
